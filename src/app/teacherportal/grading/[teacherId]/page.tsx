...
type FilterType = 'all' | 'graded' | 'pending';
...
export default function GradingPage() {
    ...
    const fetchData = async () => {
      try {
        ...
        const [classRes, assessRes] = await Promise.all([
          axios.get(`${baseurl}/classes/assigned/${teacherId}`),
          axios.get(`${baseurl}/get_assesments_by_teacher/${teacherId}`).catch(() => ({ data: { data: [] } })),
        ]);
        setAssignedClasses(classRes.data?.data || []);
  ...
  const handleViewGrading = (classroomId: string) => {
    router.push(`/teacherportal/grading/SeeAssesment/${classroomId}`);
  };

  const getAssessmentForClass = (classId: string) =>
    assessments.find(a => a.classId === classId);

  const counts = {
    all:     assignedClasses.length,
    graded:  assignedClasses.filter(c => !!getAssessmentForClass(c._id)).length,
    pending: assignedClasses.filter(c => !getAssessmentForClass(c._id)).length,
  };

  const filtered = assignedClasses.filter(cls => {
    const hasAssessment = !!getAssessmentForClass(cls._id);
    const matchFilter =
      filter === 'all' ? true :
      filter === 'graded' ? hasAssessment :
      !hasAssessment;
    ...
    <div className="min-h-screen" style={{ background: '#f4f6fb', fontFamily: "'DM Sans', sans-serif" }}>
      <style>{`
        ...
        .grade-btn {
          ...
          cursor: pointer; transition: opacity 0.16s ease, transform 0.16s ease, box-shadow 0.16s ease;
        }
        .grade-btn:hover { opacity: 0.9; transform: translateY(-1px); box-shadow: 0 6px 20px rgba(79,70,229,.25); }
        .grade-btn:active { transform: scale(0.98); }
        .grade-btn-no-assessment {
          background: #f1f5f9; color: #64748b;
          box-shadow: none;
        }
        .grade-btn-no-assessment:hover { background: #e2e8f0; box-shadow: none; transform: translateY(-1px); }
        ...
        .status-dot-green { width: 7px; height: 7px; border-radius: 50%; background: #22c55e; flex-shrink: 0; box-shadow: 0 0 0 2px rgba(34,197,94,.2); }
        .status-dot-amber { width: 7px; height: 7px; border-radius: 50%; background: #f59e0b; flex-shrink: 0; box-shadow: 0 0 0 2px rgba(245,158,11,.2); }
          ...
          <div className="header-title-row">
            <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
              <div style={{ width: 44, height: 44, borderRadius: 12, flexShrink: 0, background: 'linear-gradient(135deg,#3730a3 0%,#6366f1 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 6px 20px rgba(99,102,241,.28)' }}>
                <ClipboardCheck style={{ width: 20, height: 20, color: '#fff' }} />
              </div>
              <div>
                ...
                <p style={{ fontSize: 13, color: '#94a3b8', margin: '3px 0 0' }}>
                  Manage and submit grades for your assigned classes
                </p>
              </div>
            </div>
            ...
            {!loading && assignedClasses.length > 0 && (
              <div className="stats-strip">
                {([
                  ...
                  { label: 'Ready',     value: counts.graded,           color: '#22c55e' },
                ] as const).map(s => (
          ...
          <div className="header-filter-row">
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, flex: 1 }}>
              {([
                ...
                { key: 'graded'  as FilterType, label: 'Has Assessment', count: counts.graded  },
                { key: 'pending' as FilterType, label: 'No Assessment',  count: counts.pending },
              ]).map(f => (
                <button
                  key={f.key}
          ...
          {!loading && filtered.length > 0 && (
            <div className="cards-grid">
              {filtered.map((cls, i) => {
                ...
                const asmnt      = getAssessmentForClass(cls._id);
                const hasAsmnt   = !!asmnt;
                  ...
                  <div key={cls._id} className="card-enter" style={{ animationDelay: `${i * 45}ms` }}>
                    <div className="cls-card">
                      <div style={{ display: 'flex', flex: 1 }}>
                        ...
                        <div style={{ flex: 1, padding: '18px 18px 16px 16px', display: 'flex', flexDirection: 'column', gap: 12 }}>
                          ...
                          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                              ...
                              <span style={{
                                ...
                                padding: '3px 10px 3px 7px', borderRadius: 999, fontSize: 11,
                                fontWeight: 600, fontFamily: "'DM Mono',monospace",
                                background: hasAsmnt ? '#f0fdf4' : '#fefce8',
                                color: hasAsmnt ? '#16a34a' : '#b45309',
                              }}>
                                {hasAsmnt
                                  ? <><span className="status-dot-green" />Ready</>
                                  : <><span className="status-dot-amber" />No Assessment</>
                                }
                              </span>
                            </div>
                          ...
                          <div>
                            ...
                            {hasAsmnt && asmnt && (
                              <p style={{ fontSize: 12, color: '#94a3b8', lineHeight: 1.5, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                {asmnt.name}
                              </p>
                            )}
                            {!hasAsmnt && (
                          ...
                          {/* CTA button — always navigates */}
                          <button
                            className="grade-btn"
                            style={{ background: color.bar, color: '#fff' }}
                            onClick={() => handleViewGrading(cls._id)}
                          >
                            <><FileText style={{ width: 14, height: 14 }} /> See Grading <ArrowUpRight style={{ width: 13, height: 13, marginLeft: 2 }} /></>
                          </button>
                      ...
                      <div style={{ height: 3, background: '#f1f5f9', position: 'relative', overflow: 'hidden' }}>
                        <div style={{ position: 'absolute', inset: 0, background: color.bar, opacity: hasAsmnt ? 0.7 : 0.2, width: hasAsmnt ? '100%' : '25%', transition: 'width 0.6s ease' }} />
                      </div>
                    </div>
                  </div>
                );
          ...
          {!loading && !error && assignedClasses.length === 0 && (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '80px 20px', textAlign: 'center' }}>
              ...
              <div style={{ width: 56, height: 56, borderRadius: 16, background: 'linear-gradient(135deg,#3730a3 0%,#6366f1 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px', boxShadow: '0 8px 24px rgba(99,102,241,.25)' }}>
                <ClipboardCheck style={{ width: 24, height: 24, color: '#fff' }} />
              </div>
            ...
            <div style={{ marginTop: 32, background: 'linear-gradient(135deg,#eef2ff 0%,#f5f3ff 100%)', border: '1px solid #c7d2fe', borderRadius: 16, padding: '18px 20px', display: 'flex', alignItems: 'flex-start', gap: 14 }}>
              ...
              <div>
                ...
                <p style={{ fontSize: 13, color: '#64748b', lineHeight: 1.6 }}>
                  You have <strong style={{ color: '#4f46e5' }}>{assignedClasses.length}</strong> {assignedClasses.length === 1 ? 'class' : 'classes'} with{' '}
                  <strong style={{ color: '#22c55e' }}>{counts.graded}</strong> ready to grade and{' '}
                  <strong style={{ color: '#f59e0b' }}>{counts.pending}</strong> awaiting an assessment. Total of{' '}
                  <strong style={{ color: '#7c3aed' }}>{totalStudents}</strong> students across all classes.
                </p>
              </div>
            </div>
          )}
