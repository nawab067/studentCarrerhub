...
export default function StudentPortalSidebar({
  ...
  async function get_Student_email_Byuserid() {
    ...
    try {
      setLoading(true);
      const response = await axios.get(
        `${baseurl}/classes/student/email/${studentId}`
      );
  ...
  const initials = studentName ? studentName.slice(0, 2).toUpperCase() : "SP";

  const NavItem = ({
    item,
  }: {
    item: (typeof studentNav)[0];
  }) => {
  ...
  return (
    <TooltipProvider delayDuration={0}>
      <style>{`
        ...
        .student-sidebar-root > * { position: relative; z-index: 1; }
        ...
        .student-collapse-btn:hover { background: rgba(255,255,255,0.2); color: #fff; }

        .student-nav-section-label {
        ...
        .student-profile-btn:hover { background: rgba(255,255,255,0.14); }
        ...
        @media (max-width: 767px) {
          .student-sidebar-root { transform: translateX(-100%); }
          .student-sidebar-root.mobile-open { transform: translateX(0); }
          .student-collapse-btn { display: none; }
        }
        @media (min-width: 768px) {
          .student-mobile-toggle { display: none !important; }
        }
      `}</style>
      ...
      {/* Sidebar */}
      <aside
        className={`student-sidebar-root${isMobileOpen ? " mobile-open" : ""}`}
      ...
      >
        ...
        <div className="student-sidebar-header">
          {collapsed ? (
            <div
              className="student-sidebar-logo-icon"
              style={{ margin: "0 auto" }}
            >
            ...
            <>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <div>
                  <div className="student-sidebar-logo-text">EduPortal</div>
                  <div className="student-sidebar-logo-sub">
                    Student workspace
                  </div>
                </div>
              </div>
        ...
        <ScrollArea style={{ flex: 1, padding: "12px 10px 4px" }}>
          {!collapsed && (
            <p
              className="student-nav-section-label"
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              {collapsed ? (
                ...
                <button className="student-profile-btn">
                  <div className="student-profile-avatar">{initials}</div>
            ...
            >
              ...
              >
                ...
                >
                  <KeyRound style={{ width: 14, height: 14, color: "#4f46e5" }} />
                </span>
                Change password
              </DropdownMenuItem>

              <DropdownMenuSeparator
                style={{ margin: "4px 0", background: "#f1f5f9" }}
              />
